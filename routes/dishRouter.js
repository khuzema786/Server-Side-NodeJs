...
.get(cors.cors, (req,res,next) => {
    Dishes.find({})
    .populate('comments.author') // Using mongoose population to ensure that the client gets author info constructed from user schema
    .then((dishes) => {
        res.statusCode = 200;
...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((dish) => {
        if (dish != null) {
            req.body.author = req.user._id; // req.user is what we get with passport's authenticate.verifyUser
            dish.comments.push(req.body);
            dish.save()
            .then((dish) => {
                Dishes.findById(dish._id)
                .populate('comments.author') // Moongoose population for comments
                .then((dish) => {
                    res.statusCode = 200;
                    res.setHeader('Content-Type', 'application/json');
                    res.json(dish);
...
.put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((dish) => {
        if (dish != null && dish.comments.id(req.params.commentId) != null) {
            ...
            if (!(dish.comments.id(req.params.commentId).author).equals(req.user._id)) {    // Only user can update their own comments
...
.delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((dish) => {
        if (dish != null && dish.comments.id(req.params.commentId) != null) {
            ...
            if (!(dish.comments.id(req.params.commentId).author).equals(req.user._id)) {    // Only user can delete their own comments
                var err = new Error('Only creator can perform this');
                err.status = 401;
                return next(err);
            }
